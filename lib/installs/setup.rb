...
# puts Gem.loaded_specs["webpacker"].version 
# puts Gem::Version.new('4.x')

def check_version_and_babel
  #should i check for evidence of webpacker:install?
  File.exists?(Rails.root.join(".babelrc")) == false \
  || (Gem.loaded_specs["webpacker"].version >= Gem::Version.new('4.x.x')) == false
end
...
if if (Gem.loaded_specs["rails"].version >= Gem::Version.new('6.x'))  # rails 6 has default javascript_pack_tag
  pack_tag = <<-eos
